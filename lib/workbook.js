...
class Workbook {
    ...
    sheetToItems(sheetName) {
        ...
        worksheet.eachRow({ includeEmpty: false }, function(row, rowNumber) {
            ...
            for (let cell of row._cells) {
                ...
                const val = cell._value.model.value;
                //console.log(val,cell._column._number,columns[cell._column._number])
                ...
                if (columns[cell._column._number]) {
                    ...
                    if (val.richText) {
                        var newVal = "";
                        for (let t of val.richText) {
                            newVal += t.text;
                        }
                        item[prop] = me.parseCell(newVal);
    ...
    async flatten() {
        const c = this.crate.getJson()["@context"];
        c.push({"@base": null});
        this.crate.json_ld = await jsonld.flatten(this.crate.getJson(), c);
        //this.crate.getJson() = await jsonld.compact(this.crate.getJson(), c);
        this.crate.index()
    ...
    async workbookToCrate() {
        ...
        this.crate.getJson()["@context"].push(this.parseContextSheet());
        await this.crate.resolveContext();
        this.crate.getJson()["@graph"] = [
