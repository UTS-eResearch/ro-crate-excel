...
class Workbook {
    ...
    addContextSheet() {
        ...
        this.contextWorksheet.columns = [
            ...
            {header: "localNamespace", key: "localNamespace", width: "60"}  
        ]
    ...
    addContextTerms(terms) {
        if(typeof terms ==='string') {
            ...
            for (let term of Object.keys(terms)) {
                ...
                this.contextWorksheet.addRow({"label": term, "URL": terms[term]});
    ...
    async flatten() {
        const context = this.crate.json_ld["@context"];
        this.crate.json_ld = await jsonld.flatten(this.crate.json_ld);
        this.crate.json_ld = await jsonld.compact(this.crate.json_ld, context);
    }
    async workbookToCrate() {
        ...
        await this.flatten();
        this.resolveLinks();
        //console.log(JSON.stringify(this.crate.json_ld, null, 2));
    }
    resolveLinks() {
